<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="项目资料" prop="xiangmuziliao">
				<file-upload
					tip="点击上传项目资料"
					action="file/upload"
					:limit="1"
			...
			<el-form-item class="add-item" label="申请资金" prop="shenqingzijin">
				<el-input-number v-model="ruleForm.shenqingzijin" placeholder="申请资金" :disabled=" false ||ro.shenqingzijin"></el-input-number>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`butieshenqing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`butieshenqing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
