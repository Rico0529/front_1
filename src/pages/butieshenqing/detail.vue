	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/butieshenqing?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.shenqingzijin}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">申请内容</div>
						<div class="text "  >{{detail.shenqingneirong}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.fuzerenxingming}}</div>
					</div>
					<div class="item">
						<div class="lable">项目资料</div>
						<el-button class="uploadBtn" @click="download(detail.xiangmuziliao )">点击下载</el-button>
					</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('butieshenqing','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('butieshenqing','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('butieshenqing','发放补贴')" 
							@click="onAcross('fafangbutie','','','','')" type="warning">
							发放补贴
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('butieshenqing','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
					</div>
				</div>
			</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">申请详情</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.shenqingxiangqing"></div>
			</div>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.xiangmufengmian) {
							this.detailBanner = this.detail.xiangmufengmian.split(",w").length>1?[this.detail.xiangmufengmian]:this.detail.xiangmufengmian.split(',');
						}
						this.$forceUpdate();
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`butieshenqing`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/butieshenqing', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/butieshenqingAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此补贴申请？') .then(_ => {
					this.$http.post('butieshenqing/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
