	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/laodongbaozhang?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">保障内容</div>
						<div class="text "  >{{detail.baozhangneirong}}</div>
					</div>
					<div class="item">
						<div class="lable">最新政策</div>
						<el-button class="uploadBtn" @click="download(detail.zuixinzhengce )">点击下载</el-button>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('laodongbaozhang','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('laodongbaozhang','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('laodongbaozhang','预约')" 
							@click="onAcross('yewuyuyue','','','','')" type="warning">
							预约
						</el-button>
					</div>
				</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">保障详情</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.baozhangxiangqing"></div>
			</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`laodongbaozhang`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/laodongbaozhang', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/laodongbaozhangAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此劳动保障？') .then(_ => {
					this.$http.post('laodongbaozhang/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
