	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="类型" prop="type">
				<el-select v-model="ruleForm.type" placeholder="请选择类型" :disabled=" false  ||ro.type" >
					<el-option
			...
			<el-form-item class="add-item" label="提醒时间" prop="remindtime">
				<el-date-picker
					:disabled=" false  ||ro.remindtime"
			...
			<el-form-item class="add-item" label="简介" prop="brief">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="简介"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`popupremind/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`popupremind/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
