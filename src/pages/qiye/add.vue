<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="企业资质" v-if="type!='cross' || (type=='cross' && !ro.qiyezizhi)" prop="qiyezizhi">
				<file-upload
					tip="点击上传企业资质"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="企业资质" prop="qiyezizhi">
				<img v-if="ruleForm.qiyezizhi.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.qiyezizhi.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.qiyezizhi.split(',')" :src="baseUrl+item">
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='qiyezizhi'){
							this.ruleForm.qiyezizhi = obj[o].split(",")[0];
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`qiye/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			qiyezizhiUploadChange(fileUrls) {
				this.ruleForm.qiyezizhi = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
