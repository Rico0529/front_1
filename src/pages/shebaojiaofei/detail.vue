	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/shebaojiaofei?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">缴费日期</div>
						<div class="text "  >{{detail.jiaofeiriqi}}</div>
					</div>
					<div class="item">
					...
					<div class="item" v-if="btnAuth('shebaojiaofei','支付')">
						<div class="lable">是否支付</div>
						<div class="text">{{detail.ispay=='已支付'?'已支付':'未支付'}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('shebaojiaofei','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('shebaojiaofei','删除')" @click="delClick">删除</el-button>
						<el-button class="payBtn" v-if="(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('shebaojiaofei','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">缴费详情</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.jiaofeixiangqing"></div>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			payClick(){
				localStorage.setItem('paytable','shebaojiaofei')
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/shebaojiaofei', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/shebaojiaofeiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此社保缴费？') .then(_ => {
					this.$http.post('shebaojiaofei/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
