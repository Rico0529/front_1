<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="180px"
			>
			<el-form-item class="add-item" label="补贴名称" prop="butiemingcheng">
				<el-input v-model="ruleForm.butiemingcheng" 
					placeholder="补贴名称" clearable :disabled=" false  ||ro.butiemingcheng"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="补贴封面" v-if="type!='cross' || (type=='cross' && !ro.butiefengmian)" prop="butiefengmian">
				<file-upload
					tip="点击上传补贴封面"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="补贴封面" prop="butiefengmian">
				<img v-if="ruleForm.butiefengmian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.butiefengmian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.butiefengmian.split(',')" :src="baseUrl+item">
			</el-form-item>
			<el-form-item class="add-item" label="补贴资料" prop="butieziliao">
				<file-upload
					tip="点击上传补贴资料"
					action="file/upload"
					:limit="1"
			...
			<el-form-item class="add-item" label="补贴金额" prop="butiejine">
				<el-input-number v-model="ruleForm.butiejine" placeholder="补贴金额" :disabled=" false ||ro.butiejine"></el-input-number>
			</el-form-item>
			<el-form-item class="add-item"  label="申请状态" prop="shenqingzhuangtai">
				<el-select v-model="ruleForm.shenqingzhuangtai" placeholder="请选择申请状态" :disabled="true  ||ro.shenqingzhuangtai" >
					<el-option
			...
			<el-form-item class="add-item" label="申请内容" prop="shenqingneirong">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="申请内容"
			...
			<el-form-item class="add-item" label="申请详情" prop="shenqingxiangqing">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='butiefengmian'){
							this.ruleForm.butiefengmian = obj[o].split(",")[0];
			...
			// 多级联动参数
			info() {
				this.$http.get(`shenqingbutie/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`shenqingbutie/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			butiefengmianUploadChange(fileUrls) {
				this.ruleForm.butiefengmian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
			butieziliaoUploadChange(fileUrls) {
				this.ruleForm.butieziliao = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
