<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="技能要求" prop="jinengyaoqiu">
				<el-input v-model="ruleForm.jinengyaoqiu" 
					placeholder="技能要求" clearable :disabled=" false  ||ro.jinengyaoqiu"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="岗位薪资" prop="gangweixinzi">
				<el-input-number v-model="ruleForm.gangweixinzi" placeholder="岗位薪资" :disabled=" false ||ro.gangweixinzi"></el-input-number>
			</el-form-item>
			<el-form-item class="add-item" label="应聘岗位" prop="zhaopingangwei">
				<el-input v-model="ruleForm.zhaopingangwei" 
					placeholder="应聘岗位" clearable :disabled=" false  ||ro.zhaopingangwei"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="投递详情" prop="toudixiangqing">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`toudijianli/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`toudijianli/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
