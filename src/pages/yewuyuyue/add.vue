	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="办理时间" prop="banlishijian">
				<el-date-picker
					:disabled=" false  ||ro.banlishijian"
			...
			<el-form-item class="add-item" label="办理地点" prop="banlididian">
				<el-input v-model="ruleForm.banlididian" 
					placeholder="办理地点" clearable :disabled=" false  ||ro.banlididian"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="预约内容" prop="yuyueneirong">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="预约内容"
			...
			<el-form-item class="add-item" label="预约详情" prop="yuyuexiangqing">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`yewuyuyue/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`yewuyuyue/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
