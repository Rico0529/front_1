	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yewuyuyue?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">办理地点</div>
						<div class="text "  >{{detail.banlididian}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">预约内容</div>
						<div class="text "  >{{detail.yuyueneirong}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yewuyuyue','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yewuyuyue','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">预约详情</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.yuyuexiangqing"></div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yewuyuyue', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yewuyuyueAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此业务预约？') .then(_ => {
					this.$http.post('yewuyuyue/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
