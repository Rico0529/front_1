			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item">
					...
					<el-date-picker
						...
						type="datetimerange"
						range-separator="至"
						start-placeholder="办理时间起始"
						end-placeholder="办理时间结束"
						value-format="yyyy-MM-dd HH:mm:ss">
					</el-date-picker>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">办理地点：</div>
					<el-input v-model="formSearch.banlididian" placeholder="办理地点" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					<i class="el-icon-search"></i>
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('yewuyuyue','新增')" type="primary" @click="add('/index/yewuyuyueAdd')">
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.timeRange.length > 0) {
					searchWhere.banlishijianstart = this.timeRange[0];
					searchWhere.banlishijianend = this.timeRange[1];
				}
				if (this.formSearch.banlididian != '') searchWhere.banlididian = '%' + this.formSearch.banlididian + '%';
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`yewuyuyue/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/yewuyuyueDetail', query: params});
			},
