	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zhaopinxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
				...
				<div class="info">
					<div class="title-item">
						<div class="detail-title">
							{{detail.zhaopinmingcheng}}
						</div>
					</div>
					<div class="item">
						<div class="lable">技能要求</div>
						<div class="text "  >{{detail.jinengyaoqiu}}</div>
					</div>
					<div class="item">
						<div class="lable">岗位薪资</div>
						<div class="text "  >{{detail.gangweixinzi}}</div>
					</div>
					<div class="item">
						<div class="lable">招聘岗位</div>
						<div class="text "  >{{detail.zhaopingangwei}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">工作地点</div>
						<div class="text "  >{{detail.gongzuodidian}}</div>
					</div>
					<div class="item">
						<div class="lable">招聘时间</div>
						<div class="text "  >{{detail.zhaopinshijian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zhaopinxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zhaopinxinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('zhaopinxinxi','投递简历')" 
							@click="onAcross('toudijianli','','','','')" type="warning">
							投递简历
						</el-button>
					</div>
				</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">招聘详情</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.zhaopinxiangqing"></div>
			</div>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.zhaopinmingcheng;
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/zhaopinxinxi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			async delClick(){
				await this.$confirm('是否删除此招聘信息？') .then(_ => {
					this.$http.post('zhaopinxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
